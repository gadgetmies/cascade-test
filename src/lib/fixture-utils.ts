...
function getFixturePath(
  ...
  // Security check: ensure fixturesDir is within the project
  if (!isPathSafe(fixtureDir)) {
      throw new Error(`Security Error: Fixture directory '${fixtureDir}' is outside the current working directory or points to it.`);
  }

  const fixturePath = path.join(fixtureDir, fixtureName);

  // Security check: ensure fixturePath is within fixtureDir
  if (!isPathSafe(fixturePath, fixtureDir)) {
      throw new Error(`Security Error: Fixture path '${fixturePath}' is outside the fixtures directory.`);
  }
