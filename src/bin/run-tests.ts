...
const main = async (
  ...
  if (config.coverage?.enabled) {
    if (!isPathSafe(config.coverage.directory)) {
      console.error(`Security Error: Coverage directory '${config.coverage.directory}' is outside the current working directory or points to the CWD itself.`.red);
      process.exit(1);
    }
  ...
  if (config.outputFile) {
    if (!isPathSafe(config.outputFile)) {
      console.error(`Security Error: Output path '${config.outputFile}' is outside the current working directory or points to the CWD itself.`.red);
      process.exit(1);
    }
  }
