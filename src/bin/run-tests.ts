...
const main = async (
  ...
  if (config.coverage?.enabled) {
    if (!isPathSafe(config.coverage.directory) || path.resolve(config.coverage.directory) === process.cwd()) {
      console.error(`Security Error: Coverage directory '${config.coverage.directory}' is outside the current working directory or is the CWD itself.`.red);
      process.exit(1);
    }
  ...
  if (config.outputFile && !isPathSafe(config.outputFile)) {
    console.error(`Security Error: Output file '${config.outputFile}' is outside the current working directory.`.red);
    process.exit(1);
  }
