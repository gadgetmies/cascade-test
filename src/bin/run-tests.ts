...
cli
  ...
  .command<RunTestsCliArgv>(
    ...
    (argv: RunTestsCliArgv) => {
      ...
      if (argv.coverage && !isPathSafe(process.cwd(), coverageDir)) {
        console.error(`Security Error: Coverage directory '${coverageDir}' is outside the current working directory or points to it.`.red);
        process.exit(1);
      }

      if (argv.output && !isPathSafe(process.cwd(), argv.output)) {
        console.error(`Security Error: Output file '${argv.output}' is outside the current working directory or points to it.`.red);
        process.exit(1);
      }
