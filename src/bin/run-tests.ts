...
const main = async (
  ...
  if (config.coverage?.enabled) {
    ...
    if (!isPathSafe(process.cwd(), config.coverage.directory)) {
      console.error(
        `Security Error: Coverage directory '${config.coverage.directory}' is outside the current working directory.`
          .red
      );
      process.exit(1);
    }
  ...
  .command<RunTestsCliArgv>(
    ...
    (argv: RunTestsCliArgv) => {
      ...
      if (argv.output && !isPathSafe(process.cwd(), argv.output)) {
        console.error(
          `Security Error: Output file path '${argv.output}' is outside the current working directory.`
            .red
        );
        process.exit(1);
      }
