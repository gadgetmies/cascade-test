...
cli
  ...
  .command<RunTestsCliArgv>(
    ...
    (argv: RunTestsCliArgv) => {
      ...
      // Security Validation: Ensure output and coverage directories are safe
      const cwd = process.cwd();
      if (argv.output && !isPathSafe(cwd, argv.output)) {
        console.error(`Security Error: Output file '${argv.output}' is outside the current working directory or points to the CWD itself.`.red);
        process.exit(1);
      }
      if (coverageConfig && !isPathSafe(cwd, coverageConfig.directory)) {
        console.error(`Security Error: Coverage directory '${coverageConfig.directory}' is outside the current working directory or points to the CWD itself.`.red);
        process.exit(1);
      }
