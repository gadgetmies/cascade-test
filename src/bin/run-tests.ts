...
cli
  ...
  .command<RunTestsCliArgv>(
    ...
    (argv: RunTestsCliArgv) => {
      ...
      // Security check: ensure output file is safe if specified
      if (argv.output && !isPathSafe(argv.output)) {
        console.error(`Security Error: Output path '${argv.output}' is outside the current working directory or points to it.`.red);
        process.exit(1);
      }
