...
const main = async (
  ...
  if (config.coverage?.enabled) {
    ...
    const relative = path.relative(cwd, coverageDir);

    // Security check: Ensure coverage directory is within CWD and not CWD itself
    const isOutsideCwd = relative.startsWith("..") || path.isAbsolute(relative);
    const isCwd = relative === "";

    if (isOutsideCwd || isCwd) {
      console.error(
        `\nError: Invalid coverage directory: ${config.coverage.directory}`.red
      );
      console.error(
        `Coverage directory must be a subdirectory of the current working directory.`.red
      );
      process.exit(1);
    }
