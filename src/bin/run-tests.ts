...
const main = async (
  ...
  if (config.coverage?.enabled) {
    ...
    // Additional security check: prevent using CWD itself as coverage dir
    if (path.resolve(config.coverage.directory) === process.cwd()) {
      console.error(`Security Error: Cannot use the current working directory as the coverage directory.`.red);
      process.exit(1);
    }
  ...
  // Security check: ensure output file is safe
