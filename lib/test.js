...
${printName(node[0], style)}${
  ...
  const run = async (suite, { skippingReason, indent = 0, parentContext } = {}) => {
    try {
      ...
      for (const key of Object.keys(rest)) {
        ...
        try {
          if (R.is(Function, restElement)) {
            ...
            singleResult = await run(restElement, { skippingReason, indent: indent + 2, parentContext: setupResult })
            cancel()
          }
        } catch (e) {
